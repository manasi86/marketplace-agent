...
[![GitHub Actions][github-actions-badge]](https://github.com/your-org/marketplace-Agent/actions)
[![uv][uv-badge]](https://github.com/astral-sh/uv)
[![Nox][nox-badge]](https://github.com/wntrblm/nox)
[![Ruff][ruff-badge]](https://github.com/astral-sh/ruff)
[![Type checked with mypy][mypy-badge]](https://mypy-lang.org/)

[github-actions-badge]: https://github.com/your-org/marketplace-Agent/actions/workflows/ci.yml/badge.svg
...
Python testing, linting, and type checking tooling. The project contains a `lib` package and an
`api` package under the `src` directory.

# SQL Generator Agent

The core feature of this project is a standalone CLI agent that turns natural-language
questions into Oracle SQL. It uses [LangGraph](https://www.langchain.com/langgraph) to pipeline:

1. **Intent understanding** - parse the question into an intent, entities and a schema hint.
2. **Database connectivity check** - verify the Oracle connection.
3. **Semantic layer discovery** - auto-discover tables, views and columns across all business
   schemas from the Oracle data dictionary.
4. **SQL generation** - produce an Oracle `SELECT` statement.
5. **Validation** - enforce SELECT-only queries and run `EXPLAIN PLAN`, with a `validate ->
   generate` retry loop (up to `MAX_SQL_ATTEMPTS`, default 3) that feeds validation errors back.
6. **Execution & display** - run the query and render results as a Rich table.

Full runtime traces are shipped to a self-hosted
[Langfuse](https://langfuse.com/) instance when enabled.
...
The required variables are documented in [`.env.example`](./.env.example) and cover the
OpenAI-compatible LLM endpoint, the Oracle DSN/user/password, and the Langfuse keys and host.
Set `LANGFUSE_ENABLED=false` to disable observability.
...
configuration problems (e.g. missing credentials).
...
This will generate `dist/marketplace_agent-1.0.0.tar.gz` and `dist/marketplace_agent-1.0.0-py3-none-any.whl`.
...
```shell
uv run nox -s test -- -k test_import_packages
```
...
can be viewed at <https://your-org.github.io/marketplace-Agent/>.
...
```shell
docker build --tag marketplace-agent .
```

To run the image in a container:

```shell
docker run --rm --interactive --tty marketplace-agent
```
...
marketplace-agent v1.0.0
```

# Badge

Support [`marketplace-Agent`](https://github.com/your-org/marketplace-Agent) by adding the badge
to your project.
